You, as a Senior PHP developer, I want you to deeply analyze .docs/official directory. After than, I want you to plan a SDK implementation for this. These documentations are from https://github.com/aldinokemal/go-whatsapp-web-multidevice github repository. I want to use this package in my projects but I do not want re-write all models, validations etc. in every project.

- Design a proper abstraction layer, we **should not** depend on package's public apis or it's roots.
- We **have to** track somehow repository changes to update/upgrade our SDK. Brainstorm and design a solution.
- I want fully type-safe, production grade solution.
- Create a detailed, step-by-step development plan with atomic tasks and save it in Markdown format in the docs/plan directory.
- Create the plan using checkboxes, so you can mark off completed tasks. This makes it easier to understand which tasks have been completed and at what stage.
...
Use `BlacklineCloud\SDK\GowaPHP` as a root namespace, develop base repository elements like composer.json, pr templates etc.
